...
function MyApp({ Component, pageProps }) {
  ...
  OnClickOutsideCrypto(CryptoRef, CryptoButtonRef, () =>
    setCryptoModalOpen(false)
  );

  return (
    <div className="bg-background-dark pb-[10vh] min-h-full w-full">
      <div className="flex w-full h-[10vh] items-center justify-center">
        <Link href="/">
          <a className="text-3xl text-blue font-bold">Dante's Blog</a>
        </Link>
      </div>

      <div className="flex w-full h-full justify-center">
        <div className="flex justify-center border-4 border-foreground-dark h-content min-h-[80%] w-[80%]">
            ...
            <div className="py-2 h-content">
              <nav className="flex justify-center flex-row">
                <ul className="flex w-full justify-center text-cyan font-semibold text-lg space-x-[2vw] md:space-x-[1vw]">
                  <li>
                    <button
                      className="w-full hover:text-red"
                      ref={LinuxButtonRef}
                      onClick={() =>
                        LinuxModalOpen
                    ...
                    {LinuxModalOpen && (
                      <LinuxModal
                        LinuxRef={LinuxRef}
                        setLinuxModalOpen={setLinuxModalOpen}
                      />
                    )}
                  </li>
                  <li>
                    ...
                    {CryptoModalOpen && (
                      <CryptoModal
                        CryptoRef={CryptoRef}
                        setCryptoModalOpen={setCryptoModalOpen}
                      />
                    )}
                  </li>
                </ul>
