...
export default function ArchGuide() {
  return (
    <div className="text-foreground text-sm py-[2vh] px-[2vw] space-y-[1vh]">
      ...
      I have been running Arch since 2018 and it was my first distro, after multiple wacky installs to get something working and messing stuff up and needing to reinstall. I have 
      ...
      <h2 className="flex text-lg justify-center font-semibold">Formating the Drive <a id="format"></a></h2>
      ...
      <p>you should look for the drive you want to install sata drives will be labeled Sd(x) and nvme drives will be labeled nvme0n(x) with the x being the number or letter of the drive usually
      ...
      <p>on the drive you think you need. You should see the size of the partitions listed if the size doesn’t match up to your drive size that you are looking to install on just quit and try the next
      ...
      <p>I then delete all the partitions on the drive and over the free space I do New and I type 1G for my boot partition and then select Type and set it to EFI System. I then move down to the free
      space item again and select new and select the rest of my drive whatever the partition size it gives me after that just select write and then you can quit the program.</p>
      
      <p>I don’t run a Swap Partition as I can always create a .swapfile later on off the root partition and resize it to my liking. However I prefer to not create swapfiles due to the fact that they
      are slow and really hammer on your drive. If you have really low amounts of ram like 2GB then you should look at making a swapfile but the likely fact is anyone running that low of ram is most 
      likely running on a HDD not an SSD so swap will be even slower for them. Basically swap will allow you to use more ram by using space on the drive you assign in the case that you need it.</p>
      
      <p>People also tend to like to create a separate partition for their /home folder, It can be annoying when installing applications and needing more storage on my root and having majority 
      dedicated to /home so I don’t do it however if you do a /home partition some advantages are being able to move from Distro to Distro and just manual partition the installs to use that /home
      you already have so all your games and stuff are already there, and if your system becomes corrupt your data “should” be safe on the separate partition.</p>
      ...
      <p>if you have a SATA Drive you should see sda1 and sda2 for example. In my case I have nvme0n1p1 and nvme0n1p2 this is how linux lists partitions off drives pretty straight forward.</p>
      ...
      <p>Now we need to put some file systems on the drives for the keep the order of the partitions in your head as you will need to remember which partition is for which FS. In this case I have
      ...
      <p>just change the nvme0n1p1 to whatever drive and partition you are using such as sda1. Next I format my last partition to BTRFS now this is just what I am running at the moment I used to
      run XFS but have switched over to this for snapshots. If your new I recommend running EXT4 while XFS does tend to be slightly faster in my use case some games like WoW didnt work on it so
      it’s best to just stick to EXT4 here so your not making seperate partitions in EXT4 for some games lol. BTRFS doesn’t have this issue in my use so far. If you want to read more I have some links here for each.</p>
      ...
      <p>and that’s the partitons finished off if your running a similar setup to me If you made a seperate partiton for your /home directory just run the ame command on that partiton unless your running
      BTRFS then format your home partiton to something else like XFS or EXT4. If you created a swap partition you will need to run</p>
      ...
      <p>on your partition you want for swap most likely the x will be 3 in this case but your milage may vary. If you are making a swapfile I will show you how to make that in my Arch Tweaks gist.</p>
      ...
      <p>if you didn’t create a seperate /home partition then run if you did make a seperate partition I will get to you later on don’t worry about it for now.</p>
      ...
      <p>Then we can unmount the partiton by running</p>
      ...
      <p>So basically su is short form for the subvol command and you can also use sub and the cr command is short form for create again pretty straight forward just saving some time.</p>
      ...
      <b>@snapshots</b> - snapper will store your BTRFS Snapshots here if not using snapper this partiton is not needed
      </p>
      
      <p>So now we need to mount the partition as subvolumes… you can do so by running these commands and we might aswell setup the fstab while we are here.</p>
      ...
      <p>you may need to change the /dev/nvme0n1 command to your specific partiton if not using an NVME drive setup the same way I have mine.</p>
      ...
      <p>If you have a seperate /home partition you can now mount that for EXT4 running</p>
      ...
      <p>the options using the mount command are similar to BTRFS however you can also add barrier=0 if you would that will help performance however you may run into data loss
      in the case of a power loss if not the entire partition becoming corrupted so I left it out here if your on a laptop or a UPS maybe consider this. I will link to a section
      ...
      <p>and if you have a seperate /home directory also run</p>
      ...
      <p>You can run just mount with out -o (options) but it’s easier to apply the performance tweaks to the fstab while mounting now so that we don’t have to come back and change them later on.</p>
      
      <p><b>noatime</b> – basically improves performance by not writing the time the file was last accessed</p>
      
      <p><b>commit</b> – the time in seconds it takes for the data to be synchronized to storage set at 120 seconds here so if you lose power or crash any data within the last 2 minutes 
      ...
      <p>and if you have an seperate /home partition also run</p>
      ...
      <h2 className="flex text-lg justify-center font-semibold">Mounting Boot and setting up FSTAB (ALL) <a id="fstab"></a></h2>
      
      <p>Alright now we just have to mount the boot partiton we created earlier to /mnt we can do that by running</p>
      ...
      <p>for me its my first partition so it looks like this change yours accordingly of course. If you are running on an older EFI system that doesn’t support UEFI change /mnt/boot to /mnt/boot/efi</p>
      ...
      <p>pacstrap is similar to pacman Arch’s default package manager and it just used in the install process to install packages to the mounted directory, you can also install more stuff
      ...
      <p>genfstab is just a command to create a file that tells the kernel what partitions to mount and where -U basically just means to assign UUID Values to each.</p>
      ...
      <p>and then you should see some sort of editor and you want to make sure theres not option that looks similar to this</p>
      ...
      <p>once in the locale.gen file you will see a lot of lanaguages all you need to do is scroll down to your lanaguge in my case “en_US.UTF-8 UTF-8” and what your going to want
      to do is remove the # from the front of the line this is a comment and anything with a # in front of it will not be read, so we remove it so the language we want is read by
      the system. After thats we do Control + X and then it will ask to save the buffer we just press Y then it should ask file name to write just press enter. Now run this command
      ...
      <p>where “myhostname” is the hostname you set so in my case it would be host.localdomain host for the last line. After that’s done Control + X again press y to save and then
      ...
      <p>First we need to configue the pacman configuration file by running</p>
      ...
      using the Closed Source Nvidia driver instead of the Open Source one from nouveau as it runs games much better. I have linked a performance comparision as
      ...
      The standard nvidia driver will work fine with the default linux kernel however later on in my other guide Arch Linux Tweaks I switch to a realtime kernel and 
      other kernel like zen may not work if you try to switch down the line so its better to have DKMS from the gecko, If you don’t ever plan on running that then 
      just change to nvidia. You can read more about DKMS here - https://wiki.archlinux.org/title/Dynamic_Kernel_Module_Support Now let me explain these commands a
      little buit for those that are new, pacman is your package manager similar to apt-get or dnf. -S basically means download and install these packages and -Syyu
      basically means to update all the repos upgrade any new app versions and download and install these packages you can also run -Syyu without listing any packages.
      You can also run -Sy to just update Repos and -R to remove packages thats a basic run through of pacman. Next we will install the microcode for the CPU so run
      ...
      <p>in the 7th line where it says MODULES=() without a # infront of it your gonna want to add a few things here this is going to change depending on your GPU and FS that you chose.</p>
      
      <p>To start adding modules all you need to do is type the module between the () and then have a space between each module I have listed the modules u may need below.</p>
      ...
      <p>The first line is just used to install the GRUB Bootloader to the system, some people prefer SystemD-Boot however I find GRUB Easier to use and still use it
      in all my installs hence its being used here. The target is just saying to install a bootloader for the x86_64 Architecture type then the restis pretty straight
      forward. The config command is more useful and will be used to update your bootloader in the future if you add or remove kernels from your system to be able to
      ...
      <p>and of course username being the username would would like, Note that this username must be all lowercase letters and may only consist of numbers, letters, underscores,
      digits, periods, @ and dashes. The name may not start with a dash. The "\$" sign is allowed at the end of usernames. -mG the m means to create a new home directory for the
      user and the G means to add the user to the group specified in this case wheel. You can also delete users by running userdel -r username.</p>
      ...
      <p>Now we can move on to the final step and thats installing the Desktop Environment (DE), theres lots to chose from and Arch allows you to pretty much run any of them easily I
      will cover a few.I personally run XFCE as I like to keep my system lightweight and don’t care for many effects or built in softwares. Here are a few links to view some differents DE’s.<br />
      ...
      <p>Installing just lightdm-gtk-greeter-settings will automatically grab the dependencies lightdm and lightdm-gtk-greeter to save some typing ;)</p>
      ...
      <p>to unmount all partitions it should do this automatically on reboot anyways but always good to double check to not get corrupted or something, their may be some errors
      ...
      <p>and then GRUB Should load into your system! If something breaks or is not working the best way to work on your Arch System in the future is to keep a USB with
      an ISO on it doesn’t need to be the most up to date that way if your system breaks you can load the ISO on the USB and mount the partitons like we did in the beginning 
